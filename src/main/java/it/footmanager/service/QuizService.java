...
public class QuizService {
    ...
    public RispostaQuizResponse rispondi(RispostaQuizRequest req, Integer giocatoreId) {
        if (req.getTempoImpiegatoSec() > timeoutSec)
        ...
        boolean corretta = quiz.getRispostaCorretta().equalsIgnoreCase(req.getRispostaScelta().trim());

        RispostaGiocatore rg = new RispostaGiocatore();
        rg.setGiocatore(g); rg.setQuiz(quiz);
        rg.setEsito(corretta);
        rg.setTempoImpiegatoSec(req.getTempoImpiegatoSec());
    ...
    public List<ClassificaItemDto> classifica(Integer squadraId) {
        ...
        for (Giocatore g : lista) {
            long risp = rispostaRepo.countByGiocatore_IdAndEsitoTrue(g.getId());
    ...
    private List<BadgeDto> verificaBadge(Giocatore g) {
        long tot = rispostaRepo.countByGiocatore_IdAndEsitoTrue(g.getId());
        ...
        for (Badge b : badgeRepo.findBySogliaPuntiLessThanEqualOrderBySogliaPuntiAsc((int) tot)) {
            if (!gbRepo.existsByGiocatore_IdAndBadge_Id(g.getId(), b.getId())) {
                ...
                gbRepo.save(gb);
                nuovi.add(BadgeDto.builder().id(b.getId()).nomeBadge(b.getNomeBadge())
                        .descrizione(b.getDescrizione()).sogliaPunti(b.getSogliaPunti())
                        .immagineIcona(b.getImmagineIcona()).build());
            }
        }
        return nuovi;
    }
    ...
    private QuizDto toDto(Quiz q, boolean giaRisposto) {
        return QuizDto.builder().id(q.getId()).domanda(q.getDomanda())
                .rispostaCorretta(q.getRispostaCorretta())
